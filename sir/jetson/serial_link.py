...
class SerialLink:
    ...
    def connect(self, boot_wait: float = 4.0) -> None:
        """Open serial port and start the reader thread.

        On Linux, the CDC-ACM kernel driver asserts DTR when the port is
        opened at the OS level — pyserial's dsrdtr=False cannot prevent this.
        The Arduino Mega resets every time a new serial connection is made.
        We accept this and wait for the full boot sequence to complete
        (BNO055 init + warm-up takes ~2.5 s) before starting the reader
        thread, so no boot-banner lines pollute the response queue.
        """
        ...
        # Wait for the Arduino to finish its boot sequence.
        # BNO055.begin() + 50-sample warm-up takes ~2.5 s.
        # Add 1.5 s margin → 4 s total.
        time.sleep(boot_wait)

        # Drain every byte that arrived during boot (banner + sensor init msgs).
        while self._ser.in_waiting:
            self._ser.read(self._ser.in_waiting)
            time.sleep(0.05)   # let any trailing bytes arrive
