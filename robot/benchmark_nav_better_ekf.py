...
# ── Better EKF Uncertainty Patch ─────────────────────────────────────────
# Override the carpet-tuned default parameters with the tighter hard-floor
# values to reduce the reported EKF uncertainty.
_original_ekf_init = RobotEKF.__init__
def _tighter_ekf_init(self, q_x=0.008697, q_y=0.027119, q_th=0.004949, r_xy=0.0024, r_th=0.0006):
    _original_ekf_init(self, q_x=q_x, q_y=q_y, q_th=q_th, r_xy=r_xy, r_th=r_th)
RobotEKF.__init__ = _tighter_ekf_init
...
class BenchmarkRunner:
    ...
    def run_leg(self, goal: WaypointRef, start_label: str,
        ...
        prev_x, prev_z = sx, sz
        dist_accum = 0.0
        within_radius_since: Optional[float] = None
        ...
        while True:
            ...
            # Goal-reached check (within radius for settle_s seconds)
            d_to_goal = float(np.hypot(cx - goal.x, cz - goal.z))
            if d_to_goal <= self.goal_radius:
                if within_radius_since is None:
                    within_radius_since = now
                elif (now - within_radius_since) >= self.settle_s:
...
def plot_results(legs: List[LegData], ref_a: WaypointRef, ref_b: WaypointRef,
    ...
    ax.legend(fontsize=6, loc="best", ncol=2); ax.grid(True, alpha=0.3)
    ...
    ax.set_xlabel("Time (s)"); ax.set_ylabel("Voxel Count")
    ax.set_title("Map Growth"); ax.grid(True, alpha=0.3)

    # Plot 3: Per-Leg Error Bar Chart
    ax = axes[2]
    labels = [f"R{l['run']}\n{l['leg']}" for l in stats["per_leg"]]
    pos_errs = [l["err_pos_m"] * 100 for l in stats["per_leg"]]
    x_pos = np.arange(len(labels))
    bar_w = 0.35
    ax.bar(x_pos, pos_errs, bar_w, label='Position (cm)',
    ...
    # Plot 4: EKF Uncertainty (if applicable)
    if has_ekf:
        ax = axes[3]
        t_offset = 0.0
        for i, leg in enumerate(legs):
            ts = [s.t + t_offset for s in leg.samples if s.ekf_sigma_x is not None]
            sx = [s.ekf_sigma_x * 1000 for s in leg.samples if s.ekf_sigma_x is not None]
        ...
        ax.set_xlabel("Time (s)"); ax.set_ylabel("EKF 1σ (mm)")
